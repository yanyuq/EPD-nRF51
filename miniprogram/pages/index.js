...
Page({
  ...
  handleNotify: function(value) {
    ...
    if (this.msgIndex === 0) {
      ...
      if (data.length >= 8) {
        this.setData({
          epdPins: this.bytesToHex(data.slice(0, 7)) + (data.length > 10 ? this.bytesToHex(data.slice(10, 11)) : ''),
  ...
  sendImage: async function() {
    ...
    try {
      ...
      try {
        ...
        // 刷新显示
        this.setData({ statusText: '刷新显示...' });
        await this.write(EpdCmd.REFRESH);
        
        // 计算并显示发送时间
        const sendTime = (Date.now() - this.startTime) / 1000.0;
        this.addLog(`发送完成！耗时: ${sendTime.toFixed(2)}s`);
        this.setData({
          statusText: `发送完成！耗时: ${sendTime.toFixed(2)}s`
        });
  ...
  epdWrite: async function(cmd, data) {
    ...
    try {
      ...
      for (let i = 0; i < data.length; i += chunkSize) {
        ...
        // 根据确认间隔决定是否需要响应
        if (noReplyCount > 0) {
          await this.write(EpdCmd.SEND_DATA, chunk, false);
          noReplyCount--;
        } else {
    ...
    try {
      ...
      setTimeout(() => {
        try {
          ...
          // 更新画布 - 在微信小程序中，putImageData需要这样使用
  ...
  inputMTU: function(e) {
    ...
    if (!isNaN(mtuSize)) {
      ...
      // 如果输入为空或无效值，默认使用20
      const finalMTU = mtuSize <= 0 ? 20 : validMTU;
